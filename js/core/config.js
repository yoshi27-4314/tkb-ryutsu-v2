...
export const CONFIG = {
  ...
  // 佐川急便 契約運賃表（税別、東海発、2024/04/01〜2025/03/31）
  SAGAWA_RATES: {
...
〈北海道〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[0] || '---'}円
〈青森・秋田・岩手〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[1] || '---'}円
〈宮城・山形・福島〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[2] || '---'}円
〈茨城・栃木・群馬・埼玉・千葉・東京・神奈川・山梨〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[3] || '---'}円
〈長野・新潟〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[4] || '---'}円
〈静岡・愛知・岐阜・三重〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[5] || '---'}円
〈富山・石川・福井〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[6] || '---'}円
〈京都・滋賀・奈良・和歌山・大阪・兵庫〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[7] || '---'}円
〈岡山・広島・山口・鳥取・島根〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[8] || '---'}円
〈香川・徳島・高知・愛媛〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[9] || '---'}円
〈福岡・佐賀・長崎・大分〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[10] || '---'}円
〈熊本・宮崎・鹿児島〉 ${CONFIG.SAGAWA_RATES.rates[size]?.[11] || '---'}円
